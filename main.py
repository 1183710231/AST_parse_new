...
class AST_parse():
                    ...
                    for path, node in tree:
                        ...
                        if isinstance(node, Tree.MethodDeclaration):
                            try:
                                ...
                                self.project_pack_dict[pakage_name][class_name] = list()
                                self.project_pack_dict[pakage_name][class_name].append([node.name, maindir])
                            # self.project_pack_dict[maindir][class_name].append([node.name, [i.type.name for i in node.parameters] if node.parameters else None, node.return_type.name if node.return_type else None])
    ...
    def parse(self, dirname):
                    ...
                    for path, node in tree:
                        ...
                        if isinstance(node, Tree.Import):
                            ...
                            if not has_PackageDeclaration:
                                pakage_inside_class = self.project_pack_dict.get('')
                                class_meths_dict.update(pakage_inside_class)
                                for class_name in pakage_inside_class.keys():
                                    import_dict[class_name] = ''
                            # undo .*情况node.path没有*
                            ...
                            if self.project_pack_dict.__contains__(node.path):
                            ...
                            elif pack_dict.__contains__(node.path):
                                ...
                                if pack_dict.__contains__(pack_name):
                                    ...
                                    class_meths_dict[class_name] = self.project_pack_dict.get(pack_name).get(class_name)
                                    import_dict[class_name] = pack_name
                            ...
                            class_meths_dict.update(pakage_inside_class)
                            for class_name in pakage_inside_class.keys():
                                import_dict[class_name] = node.name
        ...
        # print(self.all_desc_path)
        # self.dump_pkl('all_desc_path.pkl', self.all_desc_path)
